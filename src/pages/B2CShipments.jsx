                  ...
                  <tr key={idx} className="hover:bg-slate-50 border-b border-slate-50 last:border-0 font-medium font-semibold text-slate-900">
                    ...
                    <td className="py-4 px-6 text-sm">
                      <div className="flex items-center justify-end gap-3 pr-12">
                        <span className="font-black text-emerald-600">{units} Units</span>
                        <button 
                ...
                {filteredShipments.filter(s => {
                  ...
                  return (
                    <div key={s.id} className="flex justify-between items-center p-4 bg-white rounded-xl border border-slate-200 shadow-sm">
                      <div>
                        <div className="font-bold text-slate-800">{s.channel} {s.isFBA && <span className="ml-1 text-[10px] bg-amber-100 text-amber-800 px-1 rounded">FBA</span>}</div>
                        <div className="text-xs text-slate-500 mb-1">{s.date} • {Array.isArray(s.whoParceled) ? s.whoParceled.join(', ') : s.whoParceled}</div>
                        <div className="text-[10px] font-bold text-emerald-600/80 uppercase tracking-widest">Via: {Array.from(new Set(contributingProducts)).join(', ')}</div>
                      </div>
